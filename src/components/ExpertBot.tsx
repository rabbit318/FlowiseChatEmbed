...
export const ExpertBot = (expertBotProps: ExpertBotProps & { class?: string }) => {
  // set a default value for showTitle if not set and merge with other props
  const props = mergeProps({ showTitle: true }, expertBotProps);
  ...
  onMount(() => {
    if (expertBotProps?.observersConfig) {
      const { observeUserInput, observeLoading, observeMessages } = expertBotProps.observersConfig;
    ...
    if (item.mime.startsWith('image/')) {
      ...
      return (
        <div
          ...
          style={{
            width: `${chatContainer ? (expertBotProps.isFullPage ? chatContainer?.offsetWidth / 4 : chatContainer?.offsetWidth / 2) : '200'}px`,
          }}
        >
    ...
    <>
      ...
      >
        ...
        <div class="flex flex-col w-full h-full justify-start z-0">
                ...
                <For each={[...starterPrompts()]}>
                  {(key) => (
                    <StarterPromptBubble
                      ...
                      starterPromptFontSize={expertBotProps.starterPromptFontSize} // Pass it here as a number
                    />
                  )}
                </For>
              </div>
          ...
          <div class="w-full px-5 pt-2 pb-1" style={{ flexShrink: 0 }}>
            {isRecording() ? (
              <>
