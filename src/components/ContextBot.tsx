...
export const ContextBot = (contextBotProps: ContextBotProps & { class?: string }) => {
  ...
  const props = mergeProps({ showTitle: true }, contextBotProps);
  ...
  onMount(() => {
    if (contextBotProps?.observersConfig) {
      const { observeUserInput, observeLoading, observeMessages } = contextBotProps.observersConfig;
    ...
    if (item.mime.startsWith('image/')) {
      ...
      return (
        <div
          ...
          style={{
            width: `${chatContainer ? (contextBotProps.isFullPage ? chatContainer?.offsetWidth / 4 : chatContainer?.offsetWidth / 2) : '200'}px`,
          }}
        >
                ...
                <For each={[...starterPrompts()]}>
                  {(key) => (
                    <StarterPromptBubble
                      ...
                      starterPromptFontSize={contextBotProps.starterPromptFontSize} // Pass it here as a number
                    />
                  )}
                </For>
              </div>
